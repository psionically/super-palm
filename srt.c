...
/* 
This program will implement the SRT scheduling algorithm.
...
If two jobs have the same remaining time, they will execute according
...
int main(){
   ...
   printf("\nEnter the input filename for SRT: ");
   scanf("%s", in_file);
   
   /* get output file from user */
   printf("Enter the output filename for SRT: ");
   scanf("%s", out_file);
   
   printf("\nThe input file for SRT is %s \nThe output file for SRT is %s \n", in_file, out_file);
   ...
   fclose(input);
   	...
   	/* sort time_needed, job_name and arrival_time fields */
   for(int i = 0; i < count; i++)
	{	
		for(int j = i + 1; j < count; j++)
		{
			if(jobs[j].time_needed < jobs[i].time_needed)
			{		
